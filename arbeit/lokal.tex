...
\subsection{... K"orper}
	...
	%normierung des mult masses: done
	
	Als lokalkompakte Gruppe exisitert nach Satz \ref{satz:topo:haarmeasure} ein Haar-Maß auf $\K_p^+$, welches wir mit $\dx_p$ bezeichnen.
	...
	Damit haben wir ein sinnvolle Normierung des additiven Maßes $\dx_p$ gefunden.
	...
	Wir k"onnen also die Normierung abh"anging von $\dx_p$ machen.
...
%fourierumkehrformel: done
		...
		Fassen wir die Normierungen des letzten Abschnitts in einer Definition zusammen.
		\begin{defi}
		...
		Die Standard-Charaktere induzieren gewissermaßen einen Isomorphismus zwischen der additiven Gruppe $\K_p^+$ und deren Dualgruppe $\widehat{\K_p^+}$. 
...
\subsubsection{Der Fall \texorpdfstring{$p < \infty$}{p kleiner unendlich}}
	...
	\begin{lemma}\label{lemma:gausssumme}
		...
		\begin{enumerate}[label=\emph{(\roman*)}]
			...
			\item Wenn $n=r$, dann 
				\begin{align*}
					\abs[g(\omega,\lambda)]^2 = c \Vol(\Zp, \dx_p) \Vol(U_n, \dxx_p)
				\end{align*}
			\item Wenn $n<r$, dann 
				\begin{align*}
					\abs[g(\omega,\lambda)]^2 = c \Vol(\Zp, \dx_p)\left[\Vol(U_n, \dxx_p) - p^{-1}\Vol(U_{r-1}\dxx_p)\right]
				\end{align*}
		\end{enumerate}
	\end{lemma}
